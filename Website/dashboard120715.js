...
$(document).ready(function () {
    ...
    $('#UserInput').submit(function(e) {
        ...
        $.ajax({
            ...
            url: 'network/eBikeDemo/object/DataVisual/stream/inputCoordinates',
            data: JSON.stringify([{ value: $(this).find('input#origin').val(), at: 1},
                                  { value: $(this).find('input#destination').val(), at: 2}]),
            dataType: "json",
...
function loadContent() {
    // Load Navigate
    var contentNavigate = $(
        '<div id="content-Navigate" class="col-md-12 sub-contents">' +
        '	 <div class="row">' +
        '	   <div class="col-md-12">' +
        '      <h2 class="sub-header">Navigate</h2>' +
        '    </div>' +
...
//        '           src="https://www.google.com/maps/embed/v1/directions?               key=AIzaSyB9YRKeHx1Y-g2VAdB5SX1rtlBkp1nrn-A&origin=Jacobs+Hall&destination=2230+Haste+St">' +
//        '       </iframe>'+
        '       </div>' +
        '	   <div class="col-md-4 content-right"></div>' +
        '  </div>' +
        '</div>');
    $('div#content').append(contentNavigate);
...
//    $('div#content').append(contentReports);
    ...
    // Load Gallery
    var contentGallery = $(
      '<div id="content-Gallery" class="col-md-12 sub-contents">'+
      '   <div class="row">'+
      '     <div class="col-md-12">'+
      ...
      '       <table style="width:100%" align="center">' +
        '       <tr>' +
        '           <td><center><img src="EnergyData.png" style="width:100%;"/></td>' +
        '           <td><center><img src="board.jpg" style="width:80%;"/></td>' +
//      '        <img src="Arduino.jpg" style="width:75%;"/>'+ 
        '       </tr>' +
        '        <tr>' +
        '           <td><center><img src="bikeChain.jpg" style="width:100%;"/></td>' +
      '             <td><center><img src="bike.jpg" style="width:83%;"/></td>' +
//      '        <img src="Moose.jpg" style="width:75%;"/>'+
        '       </tr>' + 
      '     </table>' +
      '     </div>'+
      '     <div class="col-md-4 content-right"></div>'+
      ...
      '</div>');
    contentGallery.hide();
    $('div#content').append(contentGallery);
...
/*    var contentExport = $(
    ...
    // Load Team Profile
    var contentTeam = $(
        '<div id="content-Team" class="col-md-12 sub-contents">' +
        '	 <div class="row">' +
        '	   <div class="col-md-12">' +
        '      <h2 class="sub-header">Meet the Team</h2>' +
        '    </div>' +
        '	   <div class="col-md-8 content-left">' +
        '    Preet Gill<br>Max Morrison<br>Grace Vasiknanonte<br>Katie Zheng</div>' +
        '	   <div class="col-md-4 content-right"></div>' +
        '  </div>' +
        '</div>');
    contentTeam.hide();
    $('div#content').append(contentTeam);
...
//
//  Function for loading data from the NanoServer API
//
function loadData() {
    // Request Endpoint
...
//
//  Function for loading a stream icon in data-select div
//
...
/*        subtitle: {
        ...
        yAxis: {
            title: {
                text: stream_id.substr(0, 1).toUpperCase() + stream_id.substr(1)
            }
        },
        tooltip: {
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < points.length - 1; i++) {
                    ...
                    datapoints.push([parseFloat(splitted[0]), parseFloat(splitted[1])])
